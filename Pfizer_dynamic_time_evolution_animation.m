...
for i = 1:(num_steps - 1)
    ...
    % Evaluated at the second midpoint (t+dt/2) 
    ...
    % Evaluated at the end of the interval (t+dt) 
    t_end = t_now + dt;
...
v_out.FrameRate = 15;  
v_out.Quality = 95;    % High video quality 
open(v_out);
...
for idx = 1:frame_skip:num_steps
    ...
    frame = getframe(gcf);
    frame.cdata = imresize(frame.cdata, [676, 1200]);
