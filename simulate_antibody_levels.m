function simulate_antibody_levels()
% initialize the variables
    k1 = 10.0; 
    k2 = 50.0; 
    alpha = 1.0; 
    
% Set time span of 180 days with a step size of 0.5
    tspan = 0:0.5:180; 
...
% Pre-allocate matrices to store all patient antibody data
    num_pts = length(tspan);
    pfizer_all_A = zeros(size(pfizer_data, 1), num_pts);
    moderna_all_A = zeros(size(moderna_data, 1), num_pts);
...
% Solve using ode45 
        [~, y] = ode45(@(t, y) system_odes(t, y, r1, r2, r3, r4, k1, k2, alpha, pfizer_days), tspan, y0, options);
        
% Store the antibody data for the mean calculation
        pfizer_all_A(i, :) = y(:, 1)';
        
 % Then plot A(t) 
        plot(tspan, y(:, 1), 'Color', [0.75 0.85 1.0], 'LineWidth', 0.5);
    end
    
% Calculate and plot the group mean trend
    pfizer_mean = mean(pfizer_all_A, 1);
    h_pfizer_avg = plot(tspan, pfizer_mean, 'b', 'LineWidth', 3); % We save the "handle" of the thick blue line to a variable
    
    xlabel('Days Post-Vaccination'); 
    ylabel('Antibody Level, A(t) (IU/mL)');
    title('Pfizer-BioNTech Cohort (N=11)');
    legend(h_pfizer_avg, 'Group Mean Trend', 'Location', 'best'); % We tell the legend to ONLY look at the object holding that specific nametag
    grid on;
    hold off;
...
% Solve using ode45
        [~, y] = ode45(@(t, y) system_odes(t, y, r1, r2, r3, r4, k1, k2, alpha, moderna_days), tspan, y0, options);
        
% Store the antibody data for the mean calculation
        moderna_all_A(i, :) = y(:, 1)';
        
 % Then plot A(t)
        plot(tspan, y(:, 1), 'Color', [0.75 0.95 0.75], 'LineWidth', 0.5); 
    end
    
% Calculate and plot the group mean trend
    moderna_mean = mean(moderna_all_A, 1);
    h_moderna_avg = plot(tspan, moderna_mean, 'g', 'LineWidth', 3);
    
    xlabel('Days Post-Vaccination'); 
    ylabel('Antibody Level, A(t) (IU/mL)');
    title('Moderna Cohort (N=21)');
    legend(h_moderna_avg, 'Group Mean Trend', 'Location', 'best');
    grid on;
    hold off;
    
% Main title for the entire figure
    sgtitle('Within-Host Antibody Profiles', 'FontSize', 12, 'FontWeight', 'bold');
end
